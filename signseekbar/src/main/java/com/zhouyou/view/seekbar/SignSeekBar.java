...
import static com.zhouyou.view.seekbar.SignSeekBar.TextPosition.ABOVE_SECTION_MARK;
...
import static com.zhouyou.view.seekbar.SignSeekBar.TextPosition.TOP_SIDES;
...
public class SignSeekBar extends View {
    ...
    @IntDef({NONE, SIDES, BOTTOM_SIDES, BELOW_SECTION_MARK, TOP_SIDES, ABOVE_SECTION_MARK})
    @Retention(RetentionPolicy.SOURCE)
    public @interface TextPosition {
        int SIDES = 0, BOTTOM_SIDES = 1, BELOW_SECTION_MARK = 2, TOP_SIDES = 3, ABOVE_SECTION_MARK = 4;
    }
    ...
    public SignSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (pos == 0) {
            ...
            mSectionTextPosition = TextPosition.ABOVE_SECTION_MARK;
        } else {
    ...
    private void initConfigByPriority() {
        ...
        if (isShowSectionText) {
            ...
            if (mSectionTextPosition == TextPosition.BELOW_SECTION_MARK||mSectionTextPosition == TextPosition.ABOVE_SECTION_MARK) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isShowSectionText && mSectionTextPosition >= TextPosition.BOTTOM_SIDES) { // 如果Section值在track之下或之上显示，比较取较大值
            ...
            height = Math.max(height, mThumbRadiusOnDragging * 2 + mTextSpace);// + mRectText.height()
        }
        if (isShowSign) {
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isShowSectionText) {
            ...
            } else if (mSectionTextPosition == TextPosition.BOTTOM_SIDES || mSectionTextPosition == TextPosition.BELOW_SECTION_MARK) {
            ...
            } else if (mSectionTextPosition == TextPosition.TOP_SIDES || mSectionTextPosition == TextPosition.ABOVE_SECTION_MARK) {
//                float y_ = yTop + mThumbRadiusOnDragging + mTextSpace;
                ...
                y_ += mRectText.height();
                xLeft = mLeft;
                if (mSectionTextPosition == TextPosition.TOP_SIDES) {
                    canvas.drawText(text, xLeft, y_, mPaint);
                }
                ...
                yTop = yTop + mRectText.height() + mTextSpace;
            }
        } else if (isShowThumbText && mSectionTextPosition == NONE) {
            xLeft = mLeft;
            xRight = mRight;
        ...
        boolean isShowTextBelowSectionMark = isShowSectionText
                && (mSectionTextPosition == TextPosition.BELOW_SECTION_MARK || mSectionTextPosition == TextPosition.ABOVE_SECTION_MARK);
    ...
    private void drawMark(Canvas canvas, float xLeft, float yTop, boolean isShowTextBelowSectionMark, boolean conditionInterval) {
        ...
        if (mSectionTextPosition == TextPosition.ABOVE_SECTION_MARK) {
            y_ = yTop-mRectText.height();
        }
